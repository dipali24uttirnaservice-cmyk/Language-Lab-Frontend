        ...
        item.description?.speaker_name ? (
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: sanitizeHtml(
                item.description || "No context description.",
              ),
            }}
          />
        )
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-slate-900 rounded-2xl overflow-hidden aspect-video shadow-xl border border-slate-200">
            <VideoPlayer
              ...
              poster={selectedModule.video?.thumbnail_url?.trim() || undefined}
              onEnded={onComplete}
            />
          </div>
              ...
              {selectedModule?.audio?.transcript && (
                <div className="space-y-4">
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: sanitizeHtml(selectedModule.audio.transcript),
                    }}
                  />
                </div>
              )}
          ...
          >
            <div className="bg-white border border-slate-200/80 rounded-2xl p-6 md:p-8 shadow-xl shadow-slate-200/50 space-y-6">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitizeHtml(selectedModule.content?.body || ""),
                }}
              />
      ...
      <>
        <div className="rounded-2xl border-2 border-orange-200 bg-gradient-to-br from-orange-50 to-white p-8 text-center space-y-3 mb-6">
          ...
          <p className="text-lg font-medium text-slate-500 max-w-lg mx-auto">
            ...
            {maxAttempts === 1 ? "" : "s"} for this exercise.
            {bestAttempt && (
              <>
                {" "}
                Your best score was{" "}
        ...
        <ul className="text-[15px] font-medium text-slate-600 space-y-2.5 pl-2 list-none">
          {selectedModule.shuffle_questions && (
            <li className="flex items-center gap-2">
              ...
              Questions randomized dynamically.
            </li>
          )}
          ...
          {!selectedModule.shuffle_questions &&
            !selectedModule.show_explanation && (
              <li className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300" /> Answer
                each question, then submit to see your score.
              </li>
            )}
          <li className="flex items-center gap-2">
      ...
      <div className="flex justify-center mt-2 mb-2">
        ...
        >
          ...
          <span className="relative z-10 tracking-wide">
            Start Assessment Activity Now
          </span>
          <ChevronRight
            size={22}
            className="relative z-10 stroke-[3] group-hover:translate-x-1 transition-transform"
          />
        </button>
      </div>
      <AttemptStrip attempts={attempts} onSelect={onSelectAttempt} />
  ...
  return (
    <div className="relative w-32 h-32 mx-auto">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <Award
          size={14}
          className={passed ? "text-orange-500" : "text-amber-500"}
        />
      </div>
    </div>
  );
    ...
    <div className="py-10 animate-fade-in">
      <div className="max-w-lg mx-auto">
        <div className="flex justify-center">
          <ScoreGauge
            percent={resultData?.accuracy}
            passed={resultData?.is_passed}
          />
        </div>
                ...
                <div className="flex items-start gap-3 text-xs text-amber-950 bg-amber-50/90 border border-amber-200/80 rounded-2xl p-4 shadow-xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: sanitizeHtml(r.explanation),
                      }}
                    />
                  </div>
                </div>
        ...
        {matchPairs.map((p) => {
          ...
          return (
            ...
            >
              {p.left}{" "}
              {isPaired && (
                <span className="text-orange-400">→ {pairs[p.left]}</span>
              )}
            </button>
          );
        })}
...
function ModuleListPageContent() {
  ...
  const logModuleTime = useCallback(
    async (module) => {
      ...
      const elapsed = Math.floor(
        (Date.now() - moduleStartTimeRef.current) / 1000,
      );
      ...
      try {
        ...
        console.error("Failed to log module time:", error);
      }
    },
    [type],
  );
  ...
  useEffect(() => {
    ...
    const found = modules.find((module) => module?._id === lessonId);
  ...
  }, [selectedModule, logModuleActivity]);
  ...
  useEffect(() => {
    if (
      !selectedModule ||
      (selectedModule.module_type || type) !== "exercise"
    ) {
      setAttempts([]);
      return;
    }
  ...
  const handleModuleSelection = useCallback(
    ...
    [selectedModule, logModuleActivity, searchParams, router],
  );
  ...
  }, [selectedModule, userAnswers, logModuleActivity]);
  ...
  const videoModules = useMemo(
    () =>
      modules.filter((item) => item && (item.module_type || type) === "video"),
    [modules, type],
  );

  const audioModules = useMemo(
    () =>
      modules.filter((item) => item && (item.module_type || type) === "audio"),
    [modules, type],
  );
  ...
  const exerciseModules = useMemo(
    () =>
      modules.filter(
        (item) => item && (item.module_type || type) === "exercise",
      ),
    [modules, type],
  );
  ...
  const textModules = useMemo(
    () =>
      modules.filter((item) => item && (item.module_type || type) === "text"),
    [modules, type],
  );
  ...
  const vocabularyModules = useMemo(
    () =>
      modules.filter(
        (item) => item && (item.module_type || type) === "vocabulary",
      ),
    [modules, type],
  );
